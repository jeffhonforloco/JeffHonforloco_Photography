...
const ShareModal = ({ isOpen, onClose }: ShareModalProps) => {
  ...
  const shareOnTwitter = () => {
    openShareWindow(`https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(websiteTitle)}`);
  };
  ...
  const shareOnPinterest = () => {
    openShareWindow(`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(currentUrl)}&description=${encodeURIComponent(websiteTitle)}`);
  };

  const shareOnTumblr = () => {
    openShareWindow(`https://www.tumblr.com/widgets/share/tool?posttype=link&title=${encodeURIComponent(websiteTitle)}&canonicalUrl=${encodeURIComponent(currentUrl)}`);
  };

  const shareOnWhatsApp = () => {
    openShareWindow(`https://wa.me/?text=${encodeURIComponent(websiteTitle + ' ' + currentUrl)}`);
  };
